...
couverture {i in 1..n}: sum{j in 1..n: dist [i,j]<=dc} y[j]>=1; #affectation relais si distance inf à distance de couverture
...
printf"\n";
for {j in 1..n : y[j]=1} {
	printf " Ouverture d'un relais de télévision dans la ville %s \n",  Nom[j]; #affichage de l'ouverture du relais avec le nom de la ville
printf"\n";
printf " %s couvre les villes : \n",  Nom [j]; 
	for {i in 1..n : A[i,j]=1}{
	printf " - %s \n",Nom [i]; #affichage des villes couvertes par chaque relais 
	}
printf"\n\n";}
